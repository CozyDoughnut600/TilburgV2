...
module.exports.run = async (bot, message, args) => {
    ...
    if (!message.member.hasPermission("KICK_MEMBERS")) return message.reply("Sorry, jij hebt geen permissie hiervoor..");
    ...
    if (!kickUser) return message.reply("Noem een bestaand persoon");
    ...
    var embedPrompt = new discord.MessageEmbed()
        ...
        .setDescription(`Wil jij ${kickUser} kicken?`);
    ...
    message.channel.send(embedPrompt).then(async msg => {
        ...
        if (emoji === "✅") {
            ...
            kickUser.kick(reason).catch(err => {
                if (err) return message.channel.send(`Er ging iets fout...`);
            });
            ...
            message.reply("Geen kick...").then(m => m.delete(5000));
